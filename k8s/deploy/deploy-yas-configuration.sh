...
REALM="${REALM:-Yas}"
ISSUER_URI_PUBLIC="${IDENTITY_PUBLIC_BASE_URL}/realms/${REALM}"
...
--set applicationConfig.spring.security.oauth2.resourceserver.jwt.issuer-uri="${ISSUER_URI_PUBLIC}" \
--set applicationConfig.spring.security.oauth2.resourceserver.jwt.jwk-set-uri="${JWK_SET_URI_INTERNAL}" \
...
--set backofficeBffExtraConfig.spring.security.oauth2.client.provider.keycloak.issuer-uri="${ISSUER_URI_PUBLIC}" \
...
--set storefrontBffExtraConfig.spring.security.oauth2.client.provider.keycloak.issuer-uri="${ISSUER_URI_PUBLIC}" \
